...
namespace ET.Client
{
    [UIEvent(UIType.UIHeroList)]
    public class UIHeroListEvent : AUIEvent
    {
        public override async ETTask<UI> OnCreate(Scene scene, UIComponent uiComponent)
        {
            var path = ABPathHelper.GetUGUIPath(UIType.UIHeroList);
            ...
            UI ui = uiComponent.AddChild<UI, string, GameObject>(UIType.UIHeroList, gameObject);
            ui.AddComponent<UIHeroListComponent>();
            return ui;
        }
