...
namespace ET
{
    public class WChannel: AChannel
    {
        ...
        {
            // TODO: 这里需要优化,有临时内存分配产生 GC
            var tmp = new byte[memoryBuffer.Length];
            Buffer.BlockCopy(memoryBuffer.GetBuffer(), (int)memoryBuffer.Position, tmp, (int)memoryBuffer.Position, (int)memoryBuffer.Length);
            this.webSocket.SendAsync(tmp);//, (int)memoryBuffer.Position, (int)memoryBuffer.Length);
        }
